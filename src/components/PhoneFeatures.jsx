        ...
        {features.map((item, index) => {
          ...
          return (
            <React.Fragment key={index}>
              <div
                className="cursor-pointer group flex flex-col items-center px-4"
                onClick={() => setActiveIndex(index)}
              >
                <IconComponent
                  size={28}
                  className={`mb-1 transition-colors duration-300 group-hover:text-purple-600  ${
                    isActive ? "text-purple-600" : "text-gray-800"
                  }`}
                />
                <span
                  className={`text-sm transition-colors duration-300 group-hover:text-purple-600  ${
